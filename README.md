...
|------|-------------|------|---------|:--------:|
| <a name="input_acc_bound_claims"></a> [acc\_bound\_claims](#input\_acc\_bound\_claims) | JWT/OIDC auth Method role for AWS Account in a Vault server | <pre>map(object({<br>    role_name    = string<br>    bound_claims = map(string)<br>  }))</pre> | <pre>{<br>  "key": {<br>    "bound_claims": {<br>      "project_id": "12312312",<br>      "ref": "main,develop",<br>      "ref_type": "branch"<br>    },<br>    "bound_claims_type": "glob",<br>    "role_name": "ROLE_NAME"<br>  }<br>}</pre> | no |
| <a name="input_acc_token_policies"></a> [acc\_token\_policies](#input\_acc\_token\_policies) | AWS Accounts policy name | `list(string)` | <pre>[<br>  "account_b"<br>]</pre> | no |
...
| <a name="input_create_acc_role"></a> [create\_acc\_role](#input\_create\_acc\_role) | Enable Account Role for GitLab JWT Auth Method | `bool` | `false` | no |
...
| <a name="input_create_secret_role"></a> [create\_secret\_role](#input\_create\_secret\_role) | Enable Secrets JWT Auth Method Role or not | `bool` | `false` | no |
...
| <a name="input_enabled_gl_jwt_backend"></a> [enabled\_gl\_jwt\_backend](#input\_enabled\_gl\_jwt\_backend) | Enable GitLab JWT Auth Method or not | `bool` | `false` | no |
...
| <a name="input_gh_acc_token_policies"></a> [gh\_acc\_token\_policies](#input\_gh\_acc\_token\_policies) | Policy name to read `Secrets` in path | `list(string)` | <pre>[<br>  "default"<br>]</pre> | no |
| <a name="input_gh_bound_aud"></a> [gh\_bound\_aud](#input\_gh\_bound\_aud) | URL of the repository owner, eg: `https://github.com/OWNER`, such as the organization that owns the repository. This is the only claim that can be customized | `list(string)` | <pre>[<br>  ""<br>]</pre> | no |
| <a name="input_gh_bound_sub"></a> [gh\_bound\_sub](#input\_gh\_bound\_sub) | Defines the subject claim that is to be validated by the cloud provider | `string` | `""` | no |
| <a name="input_gh_jwt_path"></a> [gh\_jwt\_path](#input\_gh\_jwt\_path) | GitHub JWT Authentication path | `string` | `"github"` | no |
...
| <a name="input_gl_jwt_path"></a> [gl\_jwt\_path](#input\_gl\_jwt\_path) | GitLab JWT Authentication path | `string` | `"jwt"` | no |
...
| <a name="input_secret_bound_claims"></a> [secret\_bound\_claims](#input\_secret\_bound\_claims) | JWT/OIDC auth Method role for Secrets values in a Vault server | <pre>map(object({<br>    role_name    = string<br>    bound_claims = map(string)<br>  }))</pre> | <pre>{<br>  "key": {<br>    "bound_claims": {<br>      "project_id": "123123",<br>      "ref": "main,develop",<br>      "ref_type": "branch"<br>    },<br>    "role_name": "reader-role"<br>  }<br>}</pre> | no |
...
| <a name="input_secret_token_policies"></a> [secret\_token\_policies](#input\_secret\_token\_policies) | Secrets policy name | `list(string)` | <pre>[<br>  "read-acc_b_creds"<br>]</pre> | no |
